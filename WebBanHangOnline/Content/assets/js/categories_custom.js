    ...
    function initPriceSlider()
    {
		$( "#slider-range" ).slider(
		{
			...
			slide: function( event, ui )
			{
				...
				var toAmount = formatNumberWithDots(ui.values[1]);
				$("#amount").val("Đ" + ui.values[0] + " - Đ" + ui.values[1]);
				$('#FromAmount').val(ui.values[0]);
				$('#ToAmount').val(ui.values[1])
			}
		});
		var initialFromAmount = formatNumberWithDots($("#slider-range").slider("values", 0));
		var initialToAmount = formatNumberWithDots($("#slider-range").slider("values", 1));
		$("#amount").val("Đ" + initialFromAmount + " - Đ" + initialToAmount);
