...
namespace WebBanHangOnline.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        [AllowAnonymous]
        public ActionResult Partial_Item_ThanhToan(OrderViewModel req)
        {
            ShoppingCart cart = (ShoppingCart)Session["Cart"];
            ViewBag.MoneyShip = req.MoneyShip;
            if (cart != null && cart.Items.Any())
            {
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    TongTien = thanhtien + req.MoneyShip;
                    ...
                    contentCustomer = contentCustomer.Replace("{{MoneyShip}}", WebBanHangOnline.Common.Common.FormatNumber(req.MoneyShip, 0));
                    ...
                    contentAdmin = contentAdmin.Replace("{{MoneyShip}}", WebBanHangOnline.Common.Common.FormatNumber(req.MoneyShip, 0));
        ...
        public ActionResult AddToCart(int id, int quantity, string size)
        {
            ...
            var checkProduct = db.ProductDetails.FirstOrDefault(x => x.Id == id);
            if (checkProduct != null)
            {
                ...
                {
                    ProductDetailId = checkProduct.Id,
                    ProductName = checkProduct.Product.Title,
                    CategoryName = checkProduct.Product.ProductCategory.Title,
                    Alias = checkProduct.Product.Alias,
                    Size = checkProduct.Size.Name,
                    Quantity = quantity
                };
                if (checkProduct.Product.ProductImage.FirstOrDefault(x => x.IsDefault) != null)
                {
                    item.ProductImg = checkProduct.Product.ProductImage.FirstOrDefault(x => x.IsDefault).Image;
                }
                item.Price = checkProduct.Product.Price;
                foreach(var i in checkProduct.Product.promotion_product)
                {
                    if (i.Type > 0)
                    {
                        item.Price -= (item.Price * i.Type) / 100;
                    }
                }
